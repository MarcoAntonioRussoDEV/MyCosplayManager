...
public record ProjectNoteResponse(UUID id, String text, Instant taskAt, Instant notifyAt, boolean done) {

    public static ProjectNoteResponse from(ProjectNote note) {
        return new ProjectNoteResponse(
                note.getId(), note.getText(), note.getTaskAt(), note.getNotifyAt(), note.isDone());
    }
}
